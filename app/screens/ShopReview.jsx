      ...
      get_done_suborders(page, 40).then((res) => {
        if (res && res.status === 200) {
          ...
          const uniqueData = newData.reduce((unique, product) => {
            if (!unique.find((item) => item.shop_id === product.shop_id)) {
              unique.push(product);
            }
            return unique;
          }, []);
          const reviewed = uniqueData.filter(
            (item) => item.is_shop_reviewed == true
          );
      ...
      get_done_suborders(page, 40).then((res) => {
        if (res && res.status === 200) {
          ...
          const unreviewed = uniqueData.filter(
            (item) => item.is_shop_reviewed == false
          );
  ...
  return (
    <SafeAreaView className="h-full w-full">
      <View className="w-full h-12 flex-row items-center justify-center mb-2 border-b-[0.5px] border-solid border-gray-300">
        ...
        <Text className="font-bold text-[16px]">Shop Reviews</Text>
      </View>
      ...
      {isLoading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View className="w-full h-fit px-2">
              <ReviewShop item={item} type="unreviewed" />
            </View>
          )}
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View className="w-full h-fit px-2">
              <ReviewShop item={item} type="reviewed" />
            </View>
          )}
          keyExtractor={(item) => item.sub_order_id}
          estimatedItemSize={30}
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View className="w-full h-fit px-2">
              <ReviewShop item={item} type="response" />
            </View>
          )}
          keyExtractor={(item) => item.sub_order_id}
          estimatedItemSize={30}
