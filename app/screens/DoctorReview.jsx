      ...
      get_done_appointments(page, 40).then((res) => {
        if (res && res.status === 200) {
          ...
          const uniqueData = newData.reduce((unique, product) => {
            if (!unique.find((item) => item.doctor.id === product.doctor.id)) {
              unique.push(product);
            }
            return unique;
          }, []);
          const reviewed = uniqueData.filter(
            (item) => item.doctor.is_reviewed === true
          );
      ...
      get_done_appointments(page, 40).then((res) => {
        if (res && res.status === 200) {
          ...
          const unreviewed = uniqueData.filter(
            (item) => item.doctor.is_reviewed === false
          );
      ...
      get_done_appointments(page, 60).then((res) => {
        if (res && res.status === 200) {
          ...
          const responsed = uniqueData.filter(
            (item) => item.doctor.doctor_responsed === true
          );
  ...
  return (
    <SafeAreaView className="h-full w-full">
      <View className="w-full h-12 flex-row items-center justify-center mb-2 border-b-[0.5px] border-solid border-gray-300">
        ...
        <Text className="font-bold text-[16px]">Doctor Reviews</Text>
      </View>
      ...
      {isLoading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View className="w-full h-fit px-2">
              <DoctorReviewSection item={item} type="unreviewed" />
            </View>
          )}
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View className="w-full h-fit px-2">
              <DoctorReviewSection item={item} type="reviewed" />
            </View>
          )}
          keyExtractor={(item) => item.appointment_id}
          estimatedItemSize={30}
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View className="w-full h-fit px-2">
              <DoctorReviewSection item={item} type="response" />
            </View>
          )}
          keyExtractor={(item) => item.appointment_id}
          estimatedItemSize={30}
