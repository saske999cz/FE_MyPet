...
import {ExperimentData} from '../../dummy/DummyData'
  ...
  return (
    <SafeAreaView className="h-full">
        <FlatList
            ...
            renderItem={({item}) => 
                (
                    <MinimalPost username={item.username} title={item.title} description={item.description} avatar={item.avatar} uploadedImage={item.uploadedImage} likes={item.likes} dislikes={item.dislikes} comments={item.comments} />)
            }
            ListHeaderComponent={() => (
                <View className="px-4 space-y-6 pb-4">
                    <View className="w-full justify-between items-center flex-row">
                        ...
                        <TouchableOpacity className="w-10 h-10 bg-[#e5e7eb] rounded-full flex-row items-center justify-center">
                            <FontAwesomeIcon icon={icons.faMagnifyingGlass} size={20} style={{color: "#000000"}}/>
                        </TouchableOpacity>
                    ...
                    <TouchableOpacity className="w-full h-10 bg-[#e5e7eb] rounded-lg flex-row items-center justify-between px-3">
                        <Text className="text-[15px] text-[#64676B]">Want to share something?</Text>
                        <FontAwesomeIcon icon={icons.faPen} size={16} style={{color: "#64676B"}}/>
                    </TouchableOpacity>
                </View>
            )}
