...
const DoctorDropDownBox = ({
        ...
        if (data) {
          const urls = await Promise.all(
            data.map(async (doctor) => {
              const folderRef = ref(FIREBASE_STORAGE, doctor.image);
              const res = await listAll(folderRef);
              return await getDownloadURL(res.items[0]);
            })
          );
  ...
  return (
    <View className="w-full h-20 flex-col">
      <View
        ...
        <View className="w-full h-12 flex-row items-center justify-start px-1">
          {selectedDoctor && selectedDoctorImage && (
            <View className="w-10 h-10 rounded-full border-[1px] border-solid border-gray-300">
              <Image
                source={{ uri: selectedDoctorImage }}
          ...
          >
            {selectedDoctor ? selectedDoctor.full_name : placeHolderText}
          </Text>
        </View>
        {isFocused && (
            ...
            {isLoading ? (
              ...
              <ScrollView className="w-[99%] h-[99%] rounded-md">
                {data.map((item, index) => (
                  <TouchableOpacity
                    onPress={() => {
                      setSelectedDoctorImage(imageList[index]);
                      setIsFocused(false);
                      onSelect(item);
                    }}
...
export default DoctorDropDownBox;
